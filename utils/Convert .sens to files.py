# -*- coding: utf-8 -*-
"""
Created on Mon Mar 21 09:06:23 2022
    ...
    with open(filename, 'rb') as f:
      ...
      for i in range(num_frames):
          ...
          if(i!=0):
               ...
               last_pose=self.frames[-1].camera_to_world.copy()
               cam_pose=frame.camera_to_world.copy()
               angle = np.arccos(((np.linalg.inv(cam_pose[:3, :3]) @ last_pose[:3, :3] @ np.array([0, 0, 1]).T) * np.array(
                    [0, 0, 1])).sum())
...
path= './data/scans_test/'

from datetime import datetime


def process(path,file):
     if(file+'.sens' in os.listdir(path+file)):
         
         sd = SensorData(path+file+'/'+file+'.sens')
         sd.export_depth_images(os.path.join(path+file+'/', 'depth'))
         sd.export_color_images(os.path.join(path+file+'/', 'color'))
         sd.export_poses(os.path.join(path+file+'/', 'pose'))
         sd.export_intrinsics(os.path.join(path+file+'/', 'intrinsic'))
         os.remove(path+file+'/'+file+'.sens')
