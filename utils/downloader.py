...
def download_release(release_scans, out_dir, file_types, use_v1_sens):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
    	for i in range(len(release_scans)):
            scan_id=release_scans[i]
            x=scan_id in os.listdir(out_dir)
            if(x):continue
            executor.submit(download_scan,scan_id,  os.path.join(out_dir, scan_id), file_types, use_v1_sens)


    """
    for i,scan_id in enumerate(release_scans):
        if(i==10):break
        print(i,scan_id)
